...
**POST `/stories`** - Create a new photo story
...
```json
{
  ...
  "visibility": "public|friends_only|close_friends_only"
}
```
...
**GET `/users/:user_id/stories`** - Get user's active stories

- Query: `limit`, `cursor`
- Response: `ListResponse<Story>`
...
**GET `/stories/:story_id/viewers`** - Get story viewers (owner only)

- Query: `limit`, `cursor`
- Response: `ListResponse<UserProfile>`

**POST `/stories/:story_id/reactions`** - Add reaction
...
**GET `/stories/:story_id/reactions`** - Get story reactions

- Response: `ListResponse<StoryReaction>`

**DELETE `/stories/:story_id/reactions`** - Remove reaction

- Response: `204 No Content`

**POST `/stories/:story_id/seen`** - Mark story as seen

- Response: `204 No Content`

**GET `/feed/stories`** - Get stories feed from followed users

- Query: `limit`, `cursor`
- Response: `ListResponse<Story>`

**GET `/stories/:story_id/metrics`** - Get story metrics (owner only)
...
```json
{
  ...
  "viewer_ids": ["uuid1", "uuid2"]
}
```

**POST `/stories/:story_id/highlights`** - Add story to highlights
...
- Response: `StoryHighlight`

**GET `/users/:user_id/highlights`** - Get user's story highlights

- Response: `ListResponse<StoryHighlight>`

**POST `/stories/media/upload`** - Upload story photo

- Body: Multipart form with image file
- Response: `StoryMedia`

**GET `/stories/media/:media_id`** - Get story photo
...
```json
{
  "status": "pending|uploaded|processing|failed|completed",
  "processed_media_id": "uuid|null"
}
```
